...
static inline void
SYS_intr_enable(void)
{
}

static inline void
SYS_intr_disable(void)
{
}
...
static inline void
ctime_idle(struct timespec clock)
{
    nanosleep(&clock, NULL);
}

static inline struct timespec
ctime_fromms(milliseconds_t ms)
{
    struct timespec ts;
    ts.tv_sec = (ms / 1000);
...
static inline bool
ctime_after(struct timespec a, struct timespec b)
{
...
static inline struct timespec
ctime_add(struct timespec a, struct timespec b)
{
    a.tv_nsec += b.tv_nsec;
    a.tv_sec += b.tv_sec;
...
static inline struct timespec
ctime_sub(struct timespec a, struct timespec b)
{
    a.tv_nsec -= b.tv_nsec;
    a.tv_sec -= b.tv_sec;
    if (a.tv_nsec < 0) {
